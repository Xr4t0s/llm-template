<template>
  <div class="flex items-center gap-2 sm:gap-3">
    ...
    >
      <!-- Gradient background -->
      <div class="absolute inset-0 bg-gradient-to-r from-indigo-600 via-indigo-500 to-cyan-500 opacity-100 group-hover:opacity-90 group-disabled:opacity-60 transition-opacity duration-300" />
      
      <!-- Shimmer effect -->
      <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full group-disabled:translate-x-0 transition-transform duration-700" />
    ...
    <div v-else class="flex items-center gap-2 sm:gap-3">
      ...
      <div class="group relative">
        <!-- Background glow -->
        <div class="absolute -inset-1 bg-gradient-to-r from-indigo-600/20 to-cyan-500/20 rounded-xl blur opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        
        <!-- Card -->
        <button
          @click="copyAddress"
          class="relative flex items-center gap-3 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 group-hover:border-indigo-400/40 group-hover:bg-white/15 transition-all duration-300 cursor-pointer"
          :title="authStore.publicKey ? authStore.publicKey : undefined"
        >
          <!-- Online indicator -->
          <div class="flex items-center gap-2">
            <div class="relative">
              <span class="w-2 h-2 rounded-full bg-gradient-to-r from-emerald-400 to-cyan-400 animate-pulse block" />
        ...
        <Transition name="fade-tooltip">
          <div
            v-if="copied"
            class="absolute top-full left-1/2 -translate-x-1/2 mt-3 px-4 py-2 rounded-lg bg-gradient-to-r from-emerald-500/30 to-cyan-500/20 border border-emerald-400/50 text-emerald-300 text-xs font-bold whitespace-nowrap shadow-lg shadow-emerald-500/20"
          >
            ✓ Address copied!
          </div>
        </Transition>
      ...
      >
        ...
        <!-- Hover background -->
        <div class="absolute inset-0 bg-gradient-to-r from-red-500/0 to-pink-500/0 group-hover:from-red-500/10 group-hover:to-pink-500/10 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300" />
